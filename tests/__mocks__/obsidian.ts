...
// Minimal UI component mocks used by FilterBar
export class ButtonComponent {
  ...
  setIcon(icon: string) { setIcon(this.buttonEl, icon); return this; }
  setTooltip(tip: string) { setTooltip(this.buttonEl, tip, { placement: 'top' }); return this; }
...
export class TextComponent {
  ...
  onChange(cb: (v: string) => void) { this.inputEl.addEventListener('input', () => cb(this.getValue())); return this; }
...
export class DropdownComponent {
  ...
  private changeHandler: ((v: string) => void) | null = null;
  constructor(container: HTMLElement) {
    this.selectEl = document.createElement('select');
    container.appendChild(this.selectEl);
    this.selectEl.addEventListener('change', () => {
      if (this.changeHandler) this.changeHandler(this.selectEl.value);
    });
  }
  ...
  addOptions(map: Record<string, string>) { Object.entries(map).forEach(([v,l]) => this.addOption(v,l)); return this; }
  setValue(v: string) { this.selectEl.value = v; return this; }
  onChange(cb: (v: string) => void) { this.changeHandler = cb; return this; }
}
...
// Simple debounce mock compatible with code usage
export function debounce<T extends (...args: any[]) => void>(fn: T, wait: number): T {
  let timeout: any;
  // @ts-ignore
  const debounced = function(this: any, ...args: any[]) {
    clearTimeout(timeout);
    timeout = setTimeout(() => fn.apply(this, args), 0); // execute next tick to keep tests fast
  } as T;
  return debounced;
}
