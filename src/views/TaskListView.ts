...
export class TaskListView extends ItemView implements OptimizedView {
	...
	async render() {
		const container = this.contentEl.createDiv({
			cls: "tasknotes-plugin tasknotes-container task-list-view-container",
		});
	...
	async createTasksContent(container: HTMLElement) {
		...
		this.filterBar.on("queryChange", async (newQuery: FilterQuery) => {
			...
			const controlsContainer = this.contentEl.querySelector(
	...
	private async refreshTasks(): Promise<void> {
		...
		try {
			...
			const { groups, hierarchicalGroups } =
				await this.plugin.filterService.getHierarchicalGroupedTasks(this.currentQuery);
	...
	renderTaskItems(container: HTMLElement, groupedTasks: Map<string, TaskInfo[]>) {
		...
		const totalTasks = Array.from(groupedTasks.values()).reduce(
		...
		if (totalTasks === 0) {
			...
			container.createEl("p", {
				text: this.plugin.i18n.translate("views.taskList.noTasksFound"),
			});
			return;
		}
	 ...
	 */
	private renderGroupedTasksWithReconciler(
		...
		groupedTasks.forEach((tasks, groupName) => {
			...
			const taskCardsContainer = groupSection.createDiv({
	 ...
	 */
	private renderHierarchicalTasks(
		container: HTMLElement,
		hierarchical: Map<string, Map<string, TaskInfo[]>>
	) {
		// Save scroll position
		const scrollTop = container.scrollTop;

		// Compute total tasks to handle empty state
		const totalTasks = Array.from(hierarchical.values()).reduce(
			(sum, subMap) =>
				sum + Array.from(subMap.values()).reduce((s, arr) => s + arr.length, 0),
			0
		);
		if (totalTasks === 0) {
			container.empty();
		...
		hierarchical.forEach((subgroups, primaryName) => {
			...
			const subtotal = Array.from(subgroups.values()).reduce(
			...
			const expandAllBtn = subgroupActions.createEl("button", {
				cls: "task-subgroup-action clickable-icon",
				attr: { "aria-label": "Expand all subgroups" },
			});
			try {
				setIcon(expandAllBtn, "list-tree");
			} catch (_) {
				expandAllBtn.textContent = "+";
			}
			const collapseAllBtn = subgroupActions.createEl("button", {
				cls: "task-subgroup-action clickable-icon",
				attr: { "aria-label": "Collapse all subgroups" },
			});
			try {
				setIcon(collapseAllBtn, "list-collapse");
			} catch (_) {
			...
			subgroups.forEach((tasks, subgroupName) => {
				...
				const subgroupHeader = subgroupSection.createEl("h4", {
					cls: "task-subgroup-header",
				});
				const subToggle = subgroupHeader.createEl("button", {
					cls: "task-subgroup-toggle",
					attr: { "aria-label": "Toggle subgroup" },
				});
				try {
					setIcon(subToggle, "chevron-right");
				} catch (_) {
					/* ignore */
				}
				const ssvg = subToggle.querySelector("svg");
				if (ssvg) {
					ssvg.classList.add("chevron");
					ssvg.setAttr("width", "14");
					ssvg.setAttr("height", "14");
				} else {
					subToggle.textContent = "▸";
					subToggle.addClass("chevron-text");
				}
	...
	private isGroupCollapsed(groupingKey: string, groupName: string): boolean {
		return GroupingUtils.isGroupCollapsed(
			TASK_LIST_VIEW_TYPE,
			groupingKey,
			groupName,
			this.plugin
	 ...
	 */
	private createClickableProjectHeader(
		headerElement: HTMLElement,
		projectName: string,
		groupStats?: { completed: number; total: number }
	): void {
