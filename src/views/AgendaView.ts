...
import { EventRef, ItemView, Setting, TFile, WorkspaceLeaf, setIcon } from 'obsidian';
...
export class AgendaView extends ItemView {
    ...
    // Collapsed state for day groups in grouped agenda
    ...
    registerEvents(): void {
        ...
        const taskUpdateListener = this.plugin.emitter.on(EVENT_TASK_UPDATED, async (payload: any) => {
    ...
    private async createAgendaControls(container: HTMLElement) {
        ...
        this.filterBar.on('expandAllGroups', () => {
            ...
            this.plugin.viewStateManager.setViewPreferences(AGENDA_VIEW_TYPE, { ...prefs, collapsedDays: next });
            const content = this.contentEl.querySelector('.agenda-view__content');
            if (content) {
                content.querySelectorAll('.agenda-view__day-header').forEach(h => h.classList.remove('is-collapsed'));
                content.querySelectorAll('[data-day-key]').forEach(el => {
                    const e = el as HTMLElement;
                    if (!e.classList.contains('agenda-view__day-header')) e.style.display = '';
                });
            }
        });
        this.filterBar.on('collapseAllGroups', () => {
            ...
            const content = this.contentEl.querySelector('.agenda-view__content');
            if (content) {
                content.querySelectorAll('.agenda-view__day-header').forEach(h => {
                ...
                content.querySelectorAll('[data-day-key]').forEach(el => {
                    const e = el as HTMLElement;
                    if (!e.classList.contains('agenda-view__day-header')) e.style.display = 'none';
                });
            }
            this.collapsedDays = next;
            this.plugin.viewStateManager.setViewPreferences(AGENDA_VIEW_TYPE, { ...prefs, collapsedDays: next });
    ...
    /**
     * Load collapsed day preferences from ViewStateManager
     */
    private loadCollapsedPrefs(): void {
        const prefs = this.plugin.viewStateManager.getViewPreferences<Record<string, unknown>>(AGENDA_VIEW_TYPE) || {};
        const stored = (prefs as any).collapsedDays;
        if (stored && typeof stored === 'object') {
            this.collapsedDays = { ...(stored as Record<string, boolean>) };
        } else {
            this.collapsedDays = {};
        }
    }

    /** Check if a specific day is collapsed */
    private isDayCollapsed(dayKey: string): boolean {
        return !!this.collapsedDays[dayKey];
    }

    /** Persist collapsed state for a day */
    private setDayCollapsed(dayKey: string, collapsed: boolean): void {
        const prefs = this.plugin.viewStateManager.getViewPreferences<Record<string, unknown>>(AGENDA_VIEW_TYPE) || {};
        const next = { ...(this.collapsedDays) } as Record<string, boolean>;
        if (collapsed) next[dayKey] = true; else delete next[dayKey];
        this.collapsedDays = next;
        this.plugin.viewStateManager.setViewPreferences(AGENDA_VIEW_TYPE, { ...(prefs as object), collapsedDays: next });
    }

    /** Show/hide all agenda items belonging to a day */
    private toggleDayItemsVisibility(dayKey: string, collapsed: boolean): void {
        const content = this.contentEl.querySelector('.agenda-view__content');
        if (!content) return;
        content.querySelectorAll('[data-day-key]').forEach(node => {
            const el = node as HTMLElement;
            if (el.dataset.dayKey === dayKey && !el.classList.contains('agenda-view__day-header')) {
    ...
    private createAgendaItemElement(item: {type: 'day-header' | 'task' | 'note', item: any, date: Date, dayKey: string}): HTMLElement {
        if (item.type === 'day-header') {
            ...
            dayHeader.dataset.dayKey = item.dayKey;

            // Create toggle button (chevron on the left)
            const toggleBtn = dayHeader.createEl('button', { cls: 'agenda-group-toggle', attr: { 'aria-label': 'Toggle day' } });
            ...
            const collapsedInitially = this.isDayCollapsed(item.dayKey);
            if (collapsedInitially) {
                dayHeader.addClass('is-collapsed');
            }
            ...
            this.registerDomEvent(dayHeader, 'click', (e: MouseEvent) => {
                ...
                const willCollapse = !dayHeader.hasClass('is-collapsed');
                this.setDayCollapsed(item.dayKey, willCollapse);
                dayHeader.toggleClass('is-collapsed', willCollapse);
                this.toggleDayItemsVisibility(item.dayKey, willCollapse);
            ...
            const el = this.createTaskItemElement(item.item as TaskInfo, item.date);
            (el as HTMLElement).dataset.dayKey = item.dayKey;
            if (this.isDayCollapsed(item.dayKey)) (el as HTMLElement).style.display = 'none';
            return el;
        } else {
            const el = this.createNoteItemElement(item.item as NoteInfo, item.date);
            (el as HTMLElement).dataset.dayKey = item.dayKey;
            if (this.isDayCollapsed(item.dayKey)) (el as HTMLElement).style.display = 'none';
            return el;
        }
    ...
    private updateAgendaItemElement(element: HTMLElement, item: {type: 'day-header' | 'task' | 'note', item: any, date: Date, dayKey: string}): void {
        if (item.type === 'day-header') {
            // Ensure dayKey is set
            (element as HTMLElement).dataset.dayKey = item.dayKey;
            ...
            const collapsed = this.isDayCollapsed(item.dayKey);
            element.classList.toggle('is-collapsed', collapsed);
            const toggleBtn = element.querySelector('.agenda-group-toggle') as HTMLElement | null;
            if (toggleBtn) toggleBtn.setAttr('aria-expanded', String(!collapsed));
        } else if (item.type === 'task') {
            // Ensure dataset key for filtering by day
            (element as HTMLElement).dataset.dayKey = item.dayKey;
            // Update card
            ...
            (element as HTMLElement).style.display = this.isDayCollapsed(item.dayKey) ? 'none' : '';
        ...
        // Note content updates are handled by its card structure
    }
