...
export class MiniCalendarView extends ItemView {
	...
	registerEvents(): void {
		...
		const dateListener = this.plugin.emitter.on(EVENT_DATE_SELECTED, (date: Date) => {
			...
			if (
				...
				if (this.colorizeMode === "daily") {
					// FIX: Use UTC methods since selectedDate is UTC-anchored
					this.plugin.cacheManager.rebuildDailyNotesCache(
						date.getUTCFullYear(),
						date.getUTCMonth()
					);
				}
		...
		const taskUpdateListener = this.plugin.emitter.on(
			EVENT_TASK_UPDATED,
			({ path, originalTask, updatedTask }) => {
				if (!path || !updatedTask) {
					console.error("EVENT_TASK_UPDATED received invalid data:", {
						path,
						originalTask,
						updatedTask,
					});
		...
		try {
			...
			if (!container) {
				...
				container = this.contentEl.createDiv({
					cls: "tasknotes-plugin mini-calendar-view",
				});
			}
	...
	private clearCalendarColorization() {
		...
		calendarDays.forEach((day) => {
			...
			day.querySelectorAll(".note-indicator, .task-indicator, .daily-note-indicator").forEach(
				(el) => el.remove()
			);

			// Remove colorization classes
			day.classList.remove(
				"mini-calendar-view__day--has-notes-few",
				"mini-calendar-view__day--has-notes-some",
				"mini-calendar-view__day--has-notes-many",
				"mini-calendar-view__day--has-tasks",
				"mini-calendar-view__day--has-completed-tasks",
				"mini-calendar-view__day--has-archived-tasks",
	...
	async colorizeCalendarForNotes() {
		return perfMonitor.measure("calendar-colorize-notes", async () => {
			...
			let calendarData;
			try {
				calendarData = await this.plugin.cacheManager.getCalendarData(
					currentYear,
					currentMonth
				);
			} catch (error) {
			...
			calendarDays.forEach((day) => {
				...
				if (dateText) {
					...
					if (noteCount > 0) {
						...
						setTooltip(indicator, `${noteCount} note${noteCount > 1 ? "s" : ""}`, {
							placement: "top",
						});
	...
	private getTaskIndicatorInfo(
		tasks: TaskInfo[] | undefined,
		dateKey: string
	): { type: "due" | "scheduled" | "completed"; count: number } | null {
	...
	async colorizeCalendarForTasks() {
		...
		const calendarData = await this.plugin.cacheManager.getCalendarData(
			currentYear,
			currentMonth
		);
		...
		calendarDays.forEach((day) => {
			...
			if (dateText) {
				...
				const indicatorInfo = this.getTaskIndicatorInfo(
					taskInfo?.tasks as TaskInfo[] | undefined,
					dateKey
				);
				...
				if (indicatorInfo) {
					...
					indicator.setAttribute(
						"aria-label",
						`${taskStatus} tasks (${indicatorInfo.count})`
					);
					setTooltip(indicator, `${taskStatus} tasks (${indicatorInfo.count})`, {
						placement: "top",
					});
	 ...
	 */
	private updateSingleCalendarCell(
		dayEl: HTMLElement,
		dateKey: string,
		tasksCache: Map<string, any>
	) {
	...
	getViewStartDate(): Date {
		...
		return createSafeDate(
			this.plugin.selectedDate.getUTCFullYear(),
			this.plugin.selectedDate.getUTCMonth(),
			1
		);
	}
	...
	getViewEndDate(): Date {
		...
		return createSafeDate(
			this.plugin.selectedDate.getUTCFullYear(),
			this.plugin.selectedDate.getUTCMonth() + 1,
			0
		);
	}
	...
	private addDropHandlers(dayEl: HTMLElement, dayDate: Date): void {
		...
		dayEl.addEventListener("drop", async (e) => {
			...
			try {
				...
				new Notice(
					`Task "${task.title}" due date set to ${format(dayDate, "MMM d, yyyy")}`
				);
