...
/**
 * The completion "modes" the context menu offers. UI-only: each resolves to a
 * concrete date here; no service method learns about "modes".
 */
 ...
 * menu can render the item disabled with a reason tooltip.
 */
...
/**
 * Resolve a completion mode to a concrete date. `asScheduled`/`onDue` report
 * unavailable (not substituting) so the caller can disable the option;
 * completion-anchored `asScheduled` resolves to `scheduled` — an explicit re-anchor.
 */
