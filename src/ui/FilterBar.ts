...
export class FilterBar extends EventEmitter {
	...
	private viewOptionsConfig: Array<{
		id: string;
		label: string;
		value: boolean;
	...
	constructor(
		...
		options?: {
			enableGroupExpandCollapse?: boolean;
			forceShowExpandCollapse?: boolean;
			viewType?: string;
		}
	) {
		super();
		...
		this.translate = (key: TranslationKey, vars?: Record<string, string>) =>
			this.plugin.i18n.translate(key, vars);
	...
	private ensureValidFilterQuery(): void {
		...
		if (
			...
			const sortKey =
				this.currentQuery?.sortKey && typeof this.currentQuery.sortKey === "string"
					? this.currentQuery.sortKey
					: "due";
			const sortDirection =
				this.currentQuery?.sortDirection &&
				typeof this.currentQuery.sortDirection === "string"
					? this.currentQuery.sortDirection
					: "asc";
			const groupKey =
				this.currentQuery?.groupKey && typeof this.currentQuery.groupKey === "string"
					? this.currentQuery.groupKey
					: "none";
	...
	private removeSearchConditions(): void {
		...
		const searchConditionIndex = this.currentQuery.children.findIndex(
			(child) =>
		...
		if (
			this.currentQuery.children.length === 1 &&
	...
	private detectActiveSavedView(): void {
		...
		if (matchingView && this.activeSavedView?.id !== matchingView.id) {
			...
			if (this.temporaryVisibleProperties && matchingView.visibleProperties) {
				...
				if (
					tempPropsSet.size === savedPropsSet.size &&
	...
	private renderTopControls(): void {
		...
		const makeViewsButton = () => {
			...
			const chevronContainer = this.viewSelectorButton.buttonEl.createDiv(
				"filter-bar__chevron-container"
			);
		...
		const makePropertiesButton = () => {
			...
			// Add text
			propertiesButton.buttonEl.createSpan({
				cls: "button-text",
				text: this.translate("ui.filterBar.properties"),
			});
		};
		const makeSearchInput = () => {
			this.searchInput = new TextComponent(topControls).setPlaceholder(
				this.translate("ui.filterBar.searchTasksPlaceholder")
			);
			this.searchInput.inputEl.addClass("filter-bar__search-input");
			setTooltip(
				this.searchInput.inputEl,
				this.translate("ui.filterBar.searchTasksTooltip"),
				{ placement: "top" }
			);
		...
		const makeSortGroupButton = () => {
			...
			// Add text
			sortGroupButton.buttonEl.createSpan({
				cls: "button-text",
				text: this.translate("ui.filterBar.sort"),
			});
		};
		const makeNewTaskButton = () => {
			...
			// Clear any existing content and build manually
			if ((newTaskButton.buttonEl as any).empty) {
				(newTaskButton.buttonEl as any).empty();
			} else {
				newTaskButton.buttonEl.innerHTML = "";
			}
			...
			// Add text
			newTaskButton.buttonEl.createSpan({
				cls: "button-text",
				text: this.translate("ui.filterBar.newTask"),
			});
		};
		...
		const dropdownClass =
			this.viewsButtonAlignment === "left"
	...
	private positionFilterBox(): void {
		...
		const filterButton = this.container.querySelector(
	...
	private handleKeyDown(event: KeyboardEvent): void {
		if (event.key === "Escape") {
			...
			if (
				...
				!this.viewSelectorDropdown.classList.contains(
					"filter-bar__view-selector-dropdown--hidden"
				)
			) {
				this.viewSelectorDropdown.classList.add(
					"filter-bar__view-selector-dropdown--hidden"
				);
				if (this.viewSelectorButton?.buttonEl) {
					this.viewSelectorButton.buttonEl.classList.remove(
						"filter-bar__templates-button--active"
					);
				}
				event.preventDefault();
			}
	...
	private handleDocumentClick(event: MouseEvent): void {
		...
		if (this.viewSelectorDropdown && this.viewSelectorButton?.buttonEl) {
			const isDropdownHidden = this.viewSelectorDropdown.classList.contains(
				"filter-bar__view-selector-dropdown--hidden"
			);
	...
	private updateFilterBoxState(): void {
		...
		if (mainBox) {
			mainBox.classList.toggle(
				...
				!this.sectionStates.filterBox
			);
		}

		if (filterToggle) {
			filterToggle.classList.toggle(
				"filter-bar__filter-toggle--active",
				this.sectionStates.filterBox
			);
		}
	}
	...
	private renderViewSelectorDropdown(): void {
		...
		if (this.savedViews.length === 0) {
			...
			const savedViewsSection = this.viewSelectorDropdown.createDiv(
				"filter-bar__view-section"
			);
	...
	private renderFilterBuilder(container: HTMLElement): void {
		...
		setTooltip(titleWrapper, "Click to expand/collapse filter conditions", {
			placement: "top",
		});
	...
	private renderFilterGroup(
		...
		setTooltip(conjunctionContainer, "Choose whether ALL or ANY of the conditions must match", {
			placement: "top",
		});
		...
		new DropdownComponent(conjunctionContainer)
			.addOption(
				"and",
				depth === 0
					? this.plugin.i18n.translate("ui.filterBar.all")
					: this.plugin.i18n.translate("ui.filterBar.all")
			)
			.addOption(
				"or",
				depth === 0
					? this.plugin.i18n.translate("ui.filterBar.any")
					: this.plugin.i18n.translate("ui.filterBar.any")
			)
	...
	private renderFilterCondition(
		...
		conditionContainer.createSpan({
			text:
				index === 0
					? this.plugin.i18n.translate("ui.filterBar.where")
		...
		setTooltip(propertyDropdown.selectEl, "Choose which task property to filter by", {
			placement: "top",
		});
		...
		setTooltip(operatorDropdown.selectEl, "Choose how to compare the property value", {
			placement: "top",
		});
	...
	private updateOperatorOptions(dropdown: DropdownComponent, property: FilterProperty): void {
		...
		const propertyDef =
			FILTER_PROPERTIES.find((p) => p.id === property) ||
			this.filterOptions.userProperties?.find((p) => p.id === property);
	...
	private renderValueInput(container: HTMLElement, condition: FilterCondition): void {
		...
		const propertyDef =
			FILTER_PROPERTIES.find((p) => p.id === condition.property) ||
			this.filterOptions.userProperties?.find((p) => p.id === condition.property);
	...
	private renderSelectInput(
		...
		const dropdown = new DropdownComponent(container).addOption("", "Select...");
		...
		setTooltip(dropdown.selectEl, `Select a ${propertyDef.label.toLowerCase()} to filter by`, {
			placement: "top",
		});
	...
	private renderDateInput(container: HTMLElement, condition: FilterCondition): void {
		...
		setTooltip(textInput.inputEl, "Enter a date using natural language or ISO format", {
			placement: "top",
		});
	...
	private renderDisplaySection(container: HTMLElement): void {
		...
		setTooltip(titleWrapper, "Click to expand/collapse sorting and grouping options", {
			placement: "top",
		});
		...
		new ButtonComponent(sortContainer)
			...
			.onClick(() => {
				this.currentQuery.sortDirection =
					this.currentQuery.sortDirection === "asc" ? "desc" : "asc";
		...
		const builtInSortOptions: Record<string, string> = {
			...
			status: this.translate("ui.filterBar.sortOptions.status"),
		...
		setTooltip(groupDropdown.selectEl, "Group tasks by a common property", {
			placement: "top",
		});
	...
	private populateViewOptions(): void {
		...
		setTooltip(titleWrapper, "Click to expand/collapse view-specific options", {
			placement: "top",
		});
	...
	private updateViewSelectorButtonState(): void {
		...
		if (this.activeSavedView) {
			this.viewSelectorButton.buttonEl.classList.add(
				"filter-bar__templates-button--saved-view-active"
			);
		} else {
			this.viewSelectorButton.buttonEl.classList.remove(
				"filter-bar__templates-button--saved-view-active"
	...
	private toggleViewSelectorDropdown(): void {
		...
		const isHidden = this.viewSelectorDropdown.classList.contains(
			"filter-bar__view-selector-dropdown--hidden"
		);
		this.viewSelectorDropdown.classList.toggle(
			"filter-bar__view-selector-dropdown--hidden",
			!isHidden
		);
	...
	private showPropertiesDropdown(event: MouseEvent): void {
		try {
			...
			this.plugin.app.workspace.trigger(
				"notice",
				this.translate("ui.filterBar.notices.propertiesMenuFailed")
			);
		}
	}
	...
	public getCurrentVisibleProperties(): string[] {
		return (
			this.temporaryVisibleProperties ||
	...
	private toggleSection(
		sectionKey: "filterBox" | "filters" | "display" | "viewOptions",
		header: HTMLElement,
		content: HTMLElement
	): void {
	...
	private updateExpandCollapseButtons(): void {
		try {
			...
			const expandBtn = this.container.querySelector(
				".filter-bar__expand-groups"
			) as HTMLElement;
			const collapseBtn = this.container.querySelector(
				".filter-bar__collapse-groups"
			) as HTMLElement;
	...
	private syncSearchInput(): void {
		...
		const searchCondition = this.currentQuery.children.find(
			(child) =>
				child.type === "condition" &&
				child.property === "title" &&
				child.operator === "contains" &&
	...
	private hasActiveFilters(): boolean {
		try {
			...
			const hasFilters =
				this.isQueryMeaningful(this.currentQuery) && this.currentQuery.children.length > 0;
	...
	private updateFilterToggleBadge(): void {
		...
		setTooltip(
			el,
			active
				? this.translate("ui.filterBar.activeFiltersTooltip")
				: this.translate("ui.filterBar.toggleFilter"),
			{ placement: "top" }
		);
	}
	 ...
	 */
	async forceRefreshOptions(filterService: {
