...
// Stub for future custom layout rendering
function createTaskCardCustomFields(container: HTMLElement, task: TaskInfo, plugin: TaskNotesPlugin): void {
    try {
        const prefs = plugin.viewStateManager?.getViewPreferences<any>('') || undefined;
        const layout = prefs?.taskCardLayout as import('../types').TaskCardLayoutConfig | undefined;
        if (!layout || !layout.rows || layout.rows.length === 0) return;

        const customFieldsRoot = document.createElement('div');
        customFieldsRoot.className = 'task-card__custom-fields';

        for (const row of layout.rows) {
            const rowEl = document.createElement('div');
            rowEl.className = 'task-card__row';
            for (const field of row.fields) {
                const fieldEl = document.createElement('span');
                fieldEl.className = 'task-card__field';
                const label = field.labelVisible ? `${field.id}: ` : '';
                let value = '';
                switch (field.id) {
                    case 'contexts':
                        value = (task.contexts || []).map(c => `@${c}`).join(' ');
                        break;
                    case 'projects':
                        value = (task.projects || []).map(p => `+${p}`).join(' ');
                        break;
                    case 'tags':
                    ...
                    case 'title':
                        ...
                        break;
                    default:
                        // user fields and other built-ins will be added later
                        value = '';
                }
                fieldEl.textContent = label + value;
                rowEl.appendChild(fieldEl);
            }
            customFieldsRoot.appendChild(rowEl);
        }

        container.appendChild(customFieldsRoot);
    } catch {}
}
...
export function createTaskCard(task: TaskInfo, plugin: TaskNotesPlugin, options: Partial<TaskCardOptions> = {}): HTMLElement {
    ...
    // Read layout (if any) from view preferences for the relevant view
    let layout: import('../types').TaskCardLayoutConfig | undefined;
    try {
        const viewType = (plugin.app.workspace.getActiveViewOfType as any)?.() ? undefined : undefined; // placeholder, leave unused for now
        const prefs = plugin.viewStateManager?.getViewPreferences<any>('') || undefined; // defensive; will be refined later
        layout = prefs?.taskCardLayout;
    } catch {}
    ...
    // Optional: render custom fields section early (keeps backward compatibility)
    try { createTaskCardCustomFields(card, task, plugin); } catch {}
...
export function updateTaskCard(element: HTMLElement, task: TaskInfo, plugin: TaskNotesPlugin, options: Partial<TaskCardOptions> = {}): void {
    ...
    // Read layout (if any) from view preferences (defensive; not yet used)
    let layout: import('../types').TaskCardLayoutConfig | undefined;
    try {
        const prefs = plugin.viewStateManager?.getViewPreferences<any>('') || undefined; // placeholder
        layout = prefs?.taskCardLayout;
    } catch {}
    ...
    // Optional: refresh custom fields section if present
    try {
        const existing = element.querySelector('.task-card__custom-fields');
        if (existing) existing.remove();
        createTaskCardCustomFields(element, task, plugin);
    } catch {}
