...
export class TaskLinkWidget extends WidgetType {
	...
	toDOM(view: EditorView): HTMLElement {
		...
		setTooltip(statusDot, `Status: ${statusConfig ? statusConfig.label : effectiveStatus}`, {
			placement: "top",
		});
		if (statusConfig) {
		...
		statusDot.addEventListener("click", async (e) => {
			...
			try {
				if (this.taskInfo.recurrence) {
					...
					const updatedTask = await this.plugin.toggleRecurringTaskComplete(
						this.taskInfo,
						targetDate
					);
					...
					const newStatusConfig =
						this.plugin.statusManager.getStatusConfig(newEffectiveStatus);
					const isNowCompleted =
						this.plugin.statusManager.isCompletedStatus(newEffectiveStatus);
					...
					const freshTask = await this.plugin.cacheManager.getTaskInfo(
						this.taskInfo.path
					);
					if (!freshTask) {
						return;
		...
		if (this.taskInfo.priority) {
			const priorityConfig = this.plugin.priorityManager.getPriorityConfig(
				this.taskInfo.priority
			);
			if (priorityConfig) {
				const priorityDot = container.createEl("span", {
					cls: "task-inline-preview__priority-dot task-card__priority-dot",
				});
				setTooltip(priorityDot, `Priority: ${priorityConfig.label} (click to change)`, {
					placement: "top",
				});
						...
						onSelect: async (newPriority) => {
							try {
								...
								const newPriorityConfig =
									this.plugin.priorityManager.getPriorityConfig(newPriority);
								if (newPriorityConfig) {
									priorityDot.style.backgroundColor = newPriorityConfig.color;
									setTooltip(
										priorityDot,
										`Priority: ${newPriorityConfig.label} (click to change)`,
										{ placement: "top" }
									);
								}
							} catch (error) {
		...
		const titleText =
			this.taskInfo.title.length > 80
				? this.taskInfo.title.slice(0, 77) + "..."
				: this.taskInfo.title;
		...
		if (this.taskInfo.due) {
			...
			const calendarIcon = dueDateSpan.createEl("span", {
				cls: "task-inline-preview__date-icon",
			});
		...
		if (
			this.taskInfo.scheduled &&
			(!this.taskInfo.due || this.taskInfo.scheduled !== this.taskInfo.due)
		) {
			...
			setTooltip(scheduledSpan, `Scheduled: ${tooltipText} (click to change)`, {
				placement: "top",
			});
		...
		if (this.taskInfo.recurrence) {
			...
			setTooltip(recurringIndicator, "Recurring task (click to change)", {
				placement: "top",
			});
			...
			recurringIndicator.addEventListener("click", (e) => {
				...
				const menu = new RecurrenceContextMenu({
					currentValue:
						typeof this.taskInfo.recurrence === "string"
							? this.taskInfo.recurrence
							: undefined,
