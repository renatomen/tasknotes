...
import { EVENT_DATA_CHANGED, EVENT_TASK_DELETED, EVENT_TASK_UPDATED, EVENT_DATE_CHANGED, TaskInfo } from '../types';
...
export class TaskCardWidget extends WidgetType {
    constructor(private plugin: TaskNotesPlugin, private task: TaskInfo, private version: number = 0) {
        super();
    }
    ...
    eq(other: TaskCardWidget): boolean {
        ...
        const taskEqual = this.task.title === other.task.title &&
                         ...
                         JSON.stringify(this.task.contexts || []) === JSON.stringify(other.task.contexts || []) &&
                         JSON.stringify(this.task.projects || []) === JSON.stringify(other.task.projects || []) &&
                         ...
                         JSON.stringify(this.task.complete_instances || []) === JSON.stringify(other.task.complete_instances || []);
    ...
    private async loadTaskForCurrentFile(view: EditorView) {
        ...
        if (file instanceof TFile) {
            try {
                ...
                const taskChanged = this.cachedTask?.title !== newTask?.title ||
