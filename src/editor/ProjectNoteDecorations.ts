...
export class ProjectSubtasksWidget extends WidgetType {
    ...
    /**
     * Compute and format the title label with completion statistics for subtasks.
     * Kept public for unit testing.
     */
    public formatSubtaskTitleForTitle(tasks: TaskInfo[]): string {
        const total = tasks.length;
        const completed = tasks.filter(t => this.plugin.statusManager.isCompletedStatus(t.status)).length;
        if (total === 0) return 'Subtasks (0)';
        const percent = Math.round((completed / total) * 100);
        const unit = total === 1 ? 'task' : 'tasks';
        return `Subtasks (${total} ${unit} • ${percent}% complete)`;
    }
    ...
    toDOM(view: EditorView): HTMLElement {
        ...
        // Update title text with completion stats using helper
        // Note: keep non-invasive to avoid altering DOM structure that tests rely on
        const completionTitle = this.formatSubtaskTitleForTitle(this.tasks);
        if (completionTitle) {
            titleEl.setText(completionTitle);
        }
