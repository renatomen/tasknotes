...
export class MigrationModal extends Modal {
	...
	async onOpen() {
		...
		new Setting(contentEl).setName(this.translate("modals.migration.title")).setHeading();
		...
		warningList.createEl("li", {
			text: this.translate("modals.migration.warnings.conversion"),
		});
		warningList.createEl("li", {
			text: this.translate("modals.migration.warnings.normalUsage"),
		});
		...
		benefitsList.createEl("li", {
			text: this.translate("modals.migration.benefits.powerfulPatterns"),
		});
		benefitsList.createEl("li", {
			text: this.translate("modals.migration.benefits.performance"),
		});
		benefitsList.createEl("li", {
			text: this.translate("modals.migration.benefits.compatibility"),
		});
		...
		const progressText = progressEl.createEl("p", {
			text: this.translate("modals.migration.progress.preparing"),
		});
		...
		const migrateButton = buttonContainer.createEl("button", {
			text: this.translate("modals.migration.buttons.migrate", {
			...
			try {
				const result = await this.migrationService.performMigration(
					(current, total, fileName) => {
						...
						progressText.textContent = this.translate(
							"modals.migration.progress.migrating",
							{ current, total, fileName }
						);
					}
				);

				progressText.textContent = this.translate("modals.migration.progress.completed", {
				...
				if (result.errors.length > 0) {
					...
					new Notice(
						this.translate("modals.migration.notices.completedWithErrors", {
					...
					new Notice(
						this.translate("modals.migration.notices.success", {
							count: result.success,
						}),
						5000
					);
				...
				progressText.textContent = this.translate("modals.migration.progress.failed", {
					error: error instanceof Error ? error.message : String(error),
				});
				new Notice(
					this.translate("modals.migration.notices.failed", {
						error: error instanceof Error ? error.message : String(error),
					}),
					10000
				);
